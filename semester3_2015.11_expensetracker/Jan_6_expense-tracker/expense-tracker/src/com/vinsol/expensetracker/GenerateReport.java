...
public class GenerateReport extends BaseActivity implements OnClickListener {
      ...
      startBtn.setOnClickListener(new View.OnClickListener() {
         public void onClick(View view) {
        	if(ok == 1)
      	      ...
      	      Toast.makeText(GenerateReport.this, "请先导出账单", Toast.LENGTH_SHORT).show();
         }
      });
	...
	public abstract class Export extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			progressDialog.setTitle("正在生成狗粮账单");
		...
		protected void onPostExecute(Void result) {
			...
			if(!isRecordAdded) {
				...
				if (resolveInfo.size() > 0) {
					Toast.makeText(GenerateReport.this, "账单生成在 "+getShowLocation(), Toast.LENGTH_LONG).show();
					startActivityForResult(intent, REQUEST_CODE);
			    } else {
			    	new AlertDialog.Builder(GenerateReport.this)
			    	.setMessage("成功导出"+getType()+"格式的账单!\n"+"账单生成在 "+getShowLocation())
			    	.setTitle("成功导出账单!")
			    	.setPositiveButton(getString(R.string.ok), new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {
//							finish();
	...
	private void  sendEmail() {
		...
		emailIntent.putExtra(Intent.EXTRA_SUBJECT, "狗粮追踪器-"+dateRange+"-账单");
		emailIntent.putExtra(Intent.EXTRA_TEXT, "以下对账单由『狗粮追踪器』自动生成！");  
		emailIntent.putExtra(Intent.EXTRA_STREAM, Uri.parse("file://"+getShowLocation() ));   
		Intent.createChooser(emailIntent, "选一个Email客户端发送吧");  
		...
		try {
		   ...
		   Toast.makeText(GenerateReport.this, "没有安装Email客户端啊！", Toast.LENGTH_SHORT).show();
		} 
	}
	...
	private class ExportPDF extends Export {
		...
		protected Void doInBackground(Void... params) {
			...
			catFont = new Font(chinese,18,Font.NORMAL);			
			subFont = new Font(Font.FontFamily.TIMES_ROMAN,16, Font.NORMAL);
	        tableHeader = new Font(chinese,14,Font.NORMAL);
	        small = new Font(chinese, 10, Font.NORMAL);
	    ...
	    private void addTable(Document document) throws DocumentException {
			...
			PdfPCell c1 = new PdfPCell(new Phrase("序号",tableHeader));
			c1.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(c1);

			c1 = new PdfPCell(new Phrase("日期",tableHeader));
			c1.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(c1);

			c1 = new PdfPCell(new Phrase("位置",tableHeader));
			c1.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(c1);
			
			c1 = new PdfPCell(new Phrase("描述",tableHeader));
			c1.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(c1);
			
			c1 = new PdfPCell(new Phrase("金额",tableHeader));
			c1.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(c1);
		...
		private void addTitle(Document document) throws DocumentException{
			...
			preface.add(new Paragraph("狗粮追踪器", catFont));
			addEmptyLine(preface, 1);
		...
		// add metadata to the PDF which can be viewed in your Adobe Reader
	 	...
	 	private void addMetaData(Document document) {
	 		document.addTitle("狗粮追踪器-账单导出");
	 		document.addSubject("本账单由『狗粮追踪器』导出");
	 		document.addKeywords("Android, PDF, 狗粮追踪器, 记账, 记账单");
	 		document.addAuthor("King Team");
	 		document.addCreator("King Team");
	 	}
		...
		private void addTotalAmountRow(PdfPTable table) {
			...
			// Adding location
			table.addCell("");
			PdfPCell Total_font = new PdfPCell(new Phrase("总金额",tableHeader));
			
			// Adding description
			table.addCell(Total_font);
		...
		private void addEmptyLine(Paragraph paragraph, int number) {
	 	...
	 	public class HeaderAndFooter extends PdfPageEventHelper {
	 		...
	 		public HeaderAndFooter() {
	 			...
	 			Chunk chunk = new Chunk("*此账单由『狗粮追踪器』生成");
